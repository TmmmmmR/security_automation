...
class PyCheckmarx(object):
	...
	def __init__(self, username, password, url):
		...
		(self.sessionId, self.client) = self.getSessionId(self.serviceUrl)
		return None
	...
	def openConnection(self):
		try:
			proxy_settings = dict(https=os.environ['http_proxy'])
	...
	def getServiceUrl(self, client):
		try:
			...
			if responseDiscovery.IsSuccesfull:
				serviceUrl = responseDiscovery.ServiceURL.replace("http", "https")
	...
	#
	def getSessionId(self, serviceUrl):
		try:
			proxy_settings = dict(https=os.environ['http_proxy'])
			clientSDK = Client(serviceUrl + "?wsdl", timeout=1200, proxy=proxy_settings)
	...
	def scanProject(self, ProjectName, ServerName, SSHFilePath, PresetID=0, GITBranch="master"):
		...
		CliScanArgs.IsIncremental = "true"
	...
	def runIncrementalScan(self, ProjectId, directory):
		config = self.client.service.GetProjectConfiguration(self.sessionId, ProjectId)
		...
		RunIncrementalScan = self.client.factory.create("RunIncrementalScan")
		RunIncrementalScan.visibleToUtherUsers = True
		RunIncrementalScan.isPublicScan = True

		tmp = self.client.service.RunIncrementalScan(self.sessionId, config.ProjectConfig.ProjectSettings,localCodeContainer,RunIncrementalScan.visibleToUtherUsers, RunIncrementalScan.isPublicScan)
